...
###################### Preppeing Data ########################
filname <- "SI_Figure_S8.pdf"

parsweep.curator <- function(filepath, trait, allee) {
  ...
  Data$Allee <- allee
  Data$Allee <- factor(Data$Allee, levels = c("present","absent"))
  Data <- melt(Data, id.vars=c("R1","R2","type","HL","param_val","Trait","Allee"),
               variable.name = "patch", value.name="density")
  Data$patch <- factor(Data$patch)
  return(Data)
}

Data.a0 <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_a0/a0/rawdata/file.RData","a0","present")
Data.b0 <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_b0/b0/rawdata/file.RData","b0","present")
Data.e <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_e/ttmean/rawdata/file.RData","e","present")
Data.p <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_p/p1/rawdata/file.RData","p","present")
Data.beta <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_beta/beta/rawdata/file.RData","beta","present")
Data.d <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_b1/b1/rawdata/file.RData","b1","present")

Data.a0.NoAl <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_a0-NoAllee/a0/rawdata/file.RData","a0","absent")
Data.b0.NoAl <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_b0-NoAllee/b0/rawdata/file.RData","b0","absent")
Data.e.NoAl <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_e-NoAllee/ttmean/rawdata/file.RData","e","absent")
Data.p.NoAl <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_p-NoAllee/p1/rawdata/file.RData","p","absent")
Data.beta.NoAl <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_beta-NoAllee/beta/rawdata/file.RData","beta","absent")
Data.d.NoAl <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_b1-NoAllee/b1/rawdata/file.RData","b1","absent")

Data <- rbind(Data.a0,Data.b0,Data.e,Data.p,Data.beta,Data.d,
              Data.a0.NoAl,Data.b0.NoAl,Data.e.NoAl,Data.p.NoAl,Data.beta.NoAl,Data.d.NoAl)
...
pobj <- ggplot(data=Data,aes(x=param_val)) + 
  ...
  scale_color_manual("Allee effect",values=c("present"="blue","absent"="red")) +
...
p.a0 <- pobj + geom_line(data=Data[Data$Trait=="a0",],aes(y=density, color=Allee, linetype=patch, linewidth=patch), alpha=0.7) +
  scale_y_continuous(expand = c(0.01,0.01)) + xlab(TeX("$a_0$"))

p.b0 <- pobj + geom_line(data=Data[Data$Trait=="b0",],aes(y=density, color=Allee, linetype=patch, linewidth=patch), alpha=0.7) +
  scale_y_continuous(expand = c(0.01,0.01)) + xlab(TeX("$b_0$"))

p.e <- pobj + geom_line(data=Data[Data$Trait=="e",],aes(y=density, color=Allee, linetype=patch, linewidth=patch), alpha=0.7) +
  scale_y_continuous(expand = c(0.01,0.01)) + xlab(TeX("$\\epsilon$"))

p.p <- pobj + geom_line(data=Data[Data$Trait=="p",],aes(y=density, color=Allee, linetype=patch, linewidth=patch), alpha=0.7) +
  scale_y_continuous(expand = c(0.01,0.01)) + xlab(TeX("$p$"))

p.beta <- pobj + geom_line(data=Data[Data$Trait=="beta",],aes(y=density, color=Allee, linetype=patch, linewidth=patch), alpha=0.7) +
  scale_y_continuous(expand = c(0.01,0.01)) + xlab(TeX("$\\beta$"))

p.D <- pobj + geom_line(data=Data[Data$Trait=="b1",],aes(y=density, color=Allee, linetype=patch, linewidth=patch), alpha=0.7) +
