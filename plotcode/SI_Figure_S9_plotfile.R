...
Data.e <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_e/ttmean/rawdata/file.RData","epsilon")
Data.ec <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_ec/ec/rawdata/file.RData","epsilon[c]")
...
Data.d <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_d/d/rawdata/file.RData","D")
Data.theta <- parsweep.curator("Data/e-var-mean_0p2-sd_0p1-parsweep_theta/theta/rawdata/file.RData","theta")

Data <- rbind(Data.a0,Data.b0,Data.e,Data.ec,Data.p,Data.beta,Data.d,Data.theta)
levels(Data$Trait)<-list("a[0]"="a0","b[0]"="b0","epsilon"=TeX("$\\epsilon$"),"epsilon[c]"=TeX("$\\epsilon_c$"),
                         p=TeX("$p$"),"beta"=TeX("$\\beta$"),"D"=TeX("$D$"),"theta"=TeX("$\\theta$"))
levels(Data$patch)<-list("N[1]"="N1","N[2]"="N2","N[1]+N[2]"="N")
rm(Data.a0,Data.b0,Data.e,Data.ec,Data.p,Data.beta,Data.d,Data.theta)
...
figure <- pobj + geom_line(aes(y=density, color=HL, linetype=type, alpha=type, linewidth=type)) + 
  facet_grid(patch~Trait,scales = "free",labeller = label_parsed, switch="both")+ xlab("Parameter Values") + ylab("Equilibrium Consumer Density") +
  scale_y_continuous(expand = c(0.01,0.01))

ggsave(filename = filname, device = "pdf", plot = figure, width = 11, height = 7)
