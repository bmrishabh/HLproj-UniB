# This file is for when the patches are independent
...
MC_TT_mut <- function(times, s, p) { #times, states, parameters
  with(as.list(c(s, p)), {
    ...
    g1   <- ((p[,"b0"]*p[,"p1"]*R1)/(p[,"b1"]+(p[,"p1"]*R1)))*(s[1:len]/2) #N1 own patch foraging
    g1cf <- 0 #N1 cross patch foraging
    g2   <- ((p[,"b0"]*p[,"p1"]*R2)/(p[,"b1"]+(p[,"p1"]*R2)))*(s[(1:len)+len]/2) #N2 own patch foraging
    g2cf <- 0 #N2 cross patch foraging
    ...
    if (p[1,"GM"] == TRUE) { #when GM is allowed
      ...
      if (p[1,"Allee"]){ #when Allee effect is allowed
        premut.dN1 <- p[,"e"]*(1 - exp(-p[1,"theta"]*(p[,"beta"]*N1)/2))*(g1+(p[,"ec"]*g1cf))
        premut.dN2 <- p[,"e"]*(1 - exp(-p[1,"theta"]*(p[,"beta"]*N2)/2))*(g2+(p[,"ec"]*g2cf))
...
GMFun <- function (p, s, N1, N2, g1, g1cf, g2, g2cf, len) {
  ...
  if (p[1,"Allee"]){ #with Allee effect
    dN1_temp <- p[,"e"]*(1 - exp(-p[1,"theta"]*(p[,"beta"]*N1)/2))*(g1+(p[,"ec"]*g1cf))
    dN2_temp <- p[,"e"]*(1 - exp(-p[1,"theta"]*(p[,"beta"]*N2)/2))*(g2+(p[,"ec"]*g2cf))
